...
main_isr:	push psw
			...
			identify:lcall delay ;now the program serves to check 
			mov a,p0 ;which key is pressed
			
			mov r0,#00h
			mov r1,#08h
			
			check_nxt: rrc  a ;key indentification logic starts here
			jc next_key
			sjmp found
			...
			found: mov a,r0; reg where key code is stored
					setb psw.4
					clr psw.3
				   mov r3,a; in r3 of reg bank 2 store direction
			//pop b
			pop acc
			pop psw
			lcall snake_game
...
dboun: ;delay subroutine for keypad
	   ;r7 and r6 of reg bank 1 used
			
			push psw
			setb psw.3 ;select reg-1 
			clr psw.4
...
org 200h;lookup tables	for char ; black=1 ;upper nibble =lower 4 bits of 8 bits of col
...
snake_game:
		...
		;r5->food position
			...
			mov r5,#05h; set the initial food position 
...
update_lcd: ;this function converts the coordinates to lcd values and displays the same
				...
				len_nt_1:
				/*mov a,@r0			;body 1 pos in a
				lcall choose_coord	;set coordinates
				mov a,@r0			;body 1 in a
				inc r0				;r0 points to body 2 coord 
				inc @r0				;body 2 in @
				subb a,18h			;compare with old head pos
				anl a,#0f0h			; if both same then horizontal movement is there
				jz s_b				; if not same head moving up r3=02 or down r3=03
